...
module.exports = {
    ...
    async execute(interaction) {
        ...
        collector.on('collect', async m => {
            if (m.content.length === 1 && /^[A-Z]$/i.test(m.content)) {
...
function createGameEmbed(username, displayedWord, incorrectGuesses, maxIncorrectGuesses) {
    ...
    let progress = `${progressEmoji.repeat(remainingGuesses)}${'🟥'.repeat(incorrectGuesses)}`;
    ...
    return new EmbedBuilder()
        ...
        .addFields(
            { name: lang.Hangman.embeds.wordToGuess, value: `\`${displayedWord.join(' ')}\``, inline: true },
            { name: lang.Hangman.embeds.guessesLeft, value: `\`${remainingGuesses}/${maxIncorrectGuesses}\``, inline: true },
            { name: lang.Hangman.embeds.progress, value: progress, inline: false }
        )
